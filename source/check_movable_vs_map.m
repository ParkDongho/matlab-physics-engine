function check_movable_vs_map
%disp('check_movable_vs_map')
global object_data
...
for a = 1 : length(object_data.section(1).map)
    ...
    penetration = polygon_to_polygon(object_data.section(1).map(a), object_data.movable(b));
    if penetration <= 0
        
        ra = facepoint - object_data.section(1).map(a).position;
        rb = facepoint - object_data.movable(b).position;
        
        rv = object_data.movable(b).velocity - object_data.section(1).map(a).velocity;
        velAlongNormal = rv * collision_normal';
        
        if velAlongNormal <= 0
            %충돌시 변화
            invMassSum = object_data.section(1).map(a).invmass + object_data.movable(b).invmass;
            
            e = min(object_data.section(1).map(a).restitution, object_data.movable(b).restitution);
            ...
            jt = -rv*t';
            jt = jt / invMassSum;
            sf = (object_data.section(1).map(a).static_friction^2 + object_data.movable(b).static_friction^2)^0.5;
            df = (object_data.section(1).map(a).dynamic_friction^2 + object_data.movable(b).dynamic_friction^2)^0.5;
        ...
        percent = 0.2;
        correction = -penetration/(object_data.section(1).map(a).invmass + object_data.movable(b).invmass)*percent*collision_normal;
